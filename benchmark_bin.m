...
% Script to compare VKF and a particle filter in a binary environment

% Parameters
lambda = 0.1;   % Volatility learning rate
sigma2 = 0.1;   % Observation noise variance
T = 200;        % Number of trials
num_particles = 10000; % Number of particles for the particle filter

% Generate a hidden state alternating between stable and volatile periods
hidden_state = zeros(T, 1);
volatility_blocks = [50, 50, 50, 50]; % Block lengths
switch_points = cumsum(volatility_blocks);

for t = 1:T
    if t <= switch_points(1)
        hidden_state(t) = 0; % Stable block 1
    elseif t <= switch_points(2)
        hidden_state(t) = 1; % Stable block 2
    elseif t <= switch_points(3)
        hidden_state(t) = 0; % Volatile block 1
    else
...
observations = double(observations > 0.5); % Convert to binary values (0 or 1)

% Apply VKF
[predictions_vkf, signals_vkf] = vkf(observations, lambda, 0.1, sigma2);

% Apply particle filter
particles = rand(num_particles, 1); % Initialize particles between 0 and 1
weights = ones(num_particles, 1) / num_particles; % Equal weights initially
predictions_pf = nan(T, 1);
volatility_pf = nan(T, 1);

for t = 1:T
    % Update particles (noisy model)
    particles = particles + sqrt(sigma2) * randn(num_particles, 1);
    particles = max(0, min(1, particles)); % Constrain particles to [0, 1]
    ...
    % Estimate prediction and volatility
